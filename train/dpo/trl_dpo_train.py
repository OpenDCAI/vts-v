...
if __name__ == "__main__":
    ...
    # Dataset
    ################
    dataset = load_dataset("json", data_files=script_args.dataset_name, num_proc=training_args.dataset_num_proc)
    
    def filter_message(example):
        if not len(example['chosen']):
            return False
        if not len(example['rejected']):
            return False
        if not len(example['prompt']):
            return False
        
        return True

    
    def map_images(example):
        if "image_path" in example:
            example["images"] = Image.open(example["image_path"]).convert("RGB").resize((224,224))
        return example

    from accelerate import PartialState
    
    with PartialState().local_main_process_first():
        dataset = dataset.filter(filter_message)
        dataset = dataset.map(map_images, num_proc=64, disable_nullable=True, remove_columns="image_path")
