...
def test_benchmark(args):
    ...
    error_log_path = os.path.join(result_save_dir, "errors_vstar.log")
    ...
    for data in tqdm(dataset, desc="Processing VStar Benchmark"):
        ...
        try:
            image_path = os.path.join(vstar_path, data['image'])
            image = Image.open(image_path)
            original_question = data['text']
            question = original_question.replace("Answer with the option's letter from the given choices directly.", "").strip()
            gold_answer = data['label']
            image_save_dir = os.path.join(image_save_dir_parent, f"{idx}")
            options = re.findall(r"\([A-Z]\) .+", question)
            all_choices = ['(A)', '(B)', '(C)', '(D)', '(E)'][:len(options)]
            developer_prompt = f"In the terminate_action, answer with the option's letter from the given choices directly."
            ...
            try:
                response, trace = run_reasoner_with_timeout(
                    ...
                    verifier,
                    [image],
                    question if args.using_vts else original_question,
                    system_prompt,
                    developer_prompt,
                    image_save_dir
                )
            ...
            print(f"Processed data id {idx} in {processing_time:.2f} seconds")
            
            prediction = analyze_answer(question, options, response, all_choices)
    ...
    accuracy = {'test': sum(1 for d in answers_output['test'] if d['gold_answer'] == d['prediction'])}
    ...
    print(f'VStar test accuracy: {round(accuracy["test"]/len(answers_output["test"])*100, 2)}%')
    ...
    accuracy_json = [{
        ...
        "len of processed data": len(answers_output['test']),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_name", type=str, default="VStar")
